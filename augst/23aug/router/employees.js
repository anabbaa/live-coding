...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(employees);//send infos assign in data employees
  } catch (err) {
...
router.post("/", async (req, res) => {
  ...
  const employee = new EmployeesData({
    name: req.body.name,//  body because its post and throw it throw row as jason
    age: req.body.age,
    add: req.body.add,
  });
  try {
...
async function getEmployee(req, res, next) {
  ...
  res.employee = employee;//i vreate new value for res object and assign it to employee
  next();
}
...
router.patch("/:name", getEmployee, async (req, res) => {
  console.log(req.body);//to know which info that client send to update
  console.log(res.employee);
  if (req.body.name) {
    res.employee.name = req.body.name; //grab the old one then updatw 
  }
  if (req.body.age) {
